...
ExN01EventAction::ExN01EventAction():fPrintModulo(2)
{}
...
void ExN01EventAction::EndOfEventAction(const G4Event* evt)
{
  ...
  if (event_id%10000 == 0)
    {
    std::cout << ">>> Event " << evt->GetEventID() << "      " << n_trajectories
       << " trajectories stored in this event." << std::endl;

    //printing survey
    /*
    if (event_id%fPrintModulo == 0)
     std::cout << "\n end of event " << std::setw(6) << event_id
             << " :" + fDecayChain << std::endl;
   */
    }
}
