...
int main(int argc, char *argv[])
{
    ...
    FILE* sino1_one;
    FILE* sino2_one;
    float *fdata,*pdata,*tdata,*sdata,*ddata,*odata,*sodata,*ssdata;
    float out;
    ...
    {
		printf("error in creating pixel_detector2.txt\n.");
		return;
	}
   ...
   for(i=0;i<nx*2;i++)
	...
	if((totaldata=fopen(pathandname,"w+"))==NULL)
	    printf("error to create totaldata.txt\n.");

  
    // Allocate memory for fdata 
	if ((fdata = (float*)malloc(nx*ny*nz*2*sizeof(float)))==NULL)
	{
     ...
     for(j=0;j<nx;j++)
	 {
      for(k=0;k<ny;k++)
	  {
	    ...
	    else
	    {
	         ...
	         fprintf(totaldata,"%f\t",*tdata++);
	    
	    if(i%2==0)
	    {
		     fprintf(detector1,"%f\t",*ddata++);
		     fprintf(pixel_detector1,"%f\n",*odata++);
		}
		else
		{
			fprintf(detector2,"%f\t",*ddata++);
			fprintf(pixel_detector2,"%f\n",*odata++);
		}
	    ...
	    if(j==64)
	    {
			if(i%2==0)
	     {
			 fprintf(sino1,"%f\t",*sdata++);
			 fprintf(sino1_one,"%f\n",*sodata++);
		 }
	     else
	     {
	         fprintf(sino2,"%f\t",*sdata++);
	         fprintf(sino2_one,"%f\n",*sodata++);
	     }
	    }
	    else
   ...
   for(i=0;i<nz*2;i++)
	{
     for(j=0;j<nx;j++)
	 {
      for(k=0;k<ny;k++)
	  {
		  ...
		  if(i%2==0)
	       {  fprintf(sino[j],"%f\t",*ssdata++); }
	      else
	       { 
	          fprintf(sino[j+nx],"%f\t",*ssdata++); }
	  }
	  if(i%2==0)
	       {  fprintf(sino[j],"\n"); }
	      else
